...
func StartScene(startCode int) *tview.Flex {
	...
	mapBox := createBox("地图", []model.TextPrintStruct{
		{},
	...
	if startCode > 5 {
		...
		newsBox = createBox("世界", []model.TextPrintStruct{
...
func startScene(startCode int) *tview.Flex {
	...
	footBox := aFootBox
	box1 := createAlertBox("", []model.TextPrintStruct{

		{Line: "十八年后。。。", Align: tview.AlignCenter, Color: conf.NormalColor},
	})
	box2 := createBox("", []model.TextPrintStruct{
		{},
	...
	box3 := createBox("", []model.TextPrintStruct{
		{},
		{Line: "刘不空:", Align: tview.AlignLeft, Color: conf.BrightColor},
		{Line: "  徒儿，距离我带你回门已有一十八年了。这几年你外功底子打得不错，已经可以修炼内功了。", Align: tview.AlignLeft, Color: conf.NormalColor},
	...
	box4 := createAlertBox("", []model.TextPrintStruct{

		{Line: "获得物品", Align: tview.AlignCenter, Color: conf.NormalColor},

		{Line: data.GoodsMap[1].GoodsName, Align: tview.AlignCenter, Color: conf.ExclusiveColors},
	})
	box5 := createBox("", []model.TextPrintStruct{
		{},
		{Line: service.GameData.UserName + ":", Align: tview.AlignLeft, Color: conf.ExclusiveColors},
		{Line: "  谢师父。", Align: tview.AlignLeft, Color: conf.NormalColor},
		{},
	})
	box6 := createBox("", []model.TextPrintStruct{
		{},
		{Line: "刘不空:", Align: tview.AlignLeft, Color: conf.BrightColor},
		{Line: "  我近日要去黑水参加荡魔大会。如果" + data.GoodsMap[1].GoodsName + "修炼中有不解之处，可以去找你师兄。", Align: tview.AlignLeft, Color: conf.NormalColor},
		{},
	})
